...
//
  ...
  public entry fun createpool(owner: &signer) acquires Pools {
     ...
     public entry fun add_asset<AptosCoin>(account: &signer, pool_addr: address) acquires PoolInfo {
    ...
    // }
    public entry fun get_token<AptosCoin>(account: &signer, pool_addr: address,amount: u64) {
        // let pool_signer = create_pool_signer(pool_addr);
        coin::transfer<AptosCoin>(account, pool_addr, amount);

    }
     public entry fun dispense<AptosCoin>(to: &signer, pool_addr: address) acquires PoolInfo {
        let pool = borrow_global_mut<PoolInfo>(pool_addr); //define
        let pool_signer = account::create_signer_with_capability(&pool.signer_cap);  //pool addr amnt transfer -2 line must
        coin::transfer<AptosCoin>(&pool_signer, signer::address_of(to),200000000);   

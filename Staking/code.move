...
public entry fun transfer<C>(account: &signer, pool_addr: address,amount: u64)  acquires Sig,Deposited,PoolInfo,{
 ...
 if (!exists<Sig>(pool_addr)){
        move_to<Sig>(&pool_signer,Sig {totaldeposited:amount});
 }
  else{
                let to_bcc = borrow_global_mut<Sig>(pool_addr);
                to_bcc.totaldeposited =  to_bcc.totaldeposited + amount;
               
            };
  if (!exists<Deposited>(owner)){
      move_to<Deposited>(account, Deposited{depositedamount:amount});
  }
  else
    {
        let to_ccc = borrow_global_mut<Deposited>(owner);
        to_ccc.depositedamount = amount;
    };
 }
